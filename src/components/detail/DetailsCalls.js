...
export default class DetailsCalls extends Component {
	...
	componentDidMount(){
		...
		axios.get(this.props.abilityUrl)
		.then(all => {
			this.setState({ 
	...
	render() {
		...
		console.log(movesList)
			...
			if(!isLoaded){
				return (
				<div className='loadingDesc'> <Loading 
				width = '12px'
				height = '12px'
				/> </div>
				)
				...
				if(this.props.abilityUrl){
					...
					<div className='moveDesc'>
						...
						<div className='moveType'>
							...
							<span className={this.renderTypes(movesList.type.name)}>{movesList.type.name} </span>
						</div>
